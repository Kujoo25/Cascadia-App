...
- **State management**: `ChatPanelProvider` context (`cascadia-web/src/lib/ai/chat-context.tsx`)
...
Three tables support AI chat persistence (defined in `cascadia-api/src/lib/db/schema/ai.ts`):
...
Defaults live in `DEFAULT_MODEL` in `cascadia-commons/src/lib/ai/model-catalog.ts` and are re-exported as `DEFAULT_MODELS` from `adapters.ts`, so there is one place to change them.

### Model Discovery

The admin model picker is populated at runtime rather than from a hardcoded list. `listProviderModels()` in `cascadia-api/src/lib/ai/model-discovery.ts` queries each provider's own list-models endpoint, and `POST /api/v1/admin/ai-settings/models` exposes it to the UI:
...
The `getAdapter()` function in `cascadia-api/src/lib/ai/adapters.ts` creates the appropriate TanStack AI adapter based on the provider configuration. It accepts a provider type, model name, API key, and optional base URL.
...
Tools are assembled in `cascadia-api/src/lib/ai/tools/index.ts`:
...
The `KnowledgeService` (`cascadia-api/src/lib/ai/KnowledgeService.ts`) makes the AI schema-aware by:
...
Every tool handler is wrapped with `withPermissionAndAudit()` (for read tools) or `withWritePermissionAndAudit()` (for write tools) from `cascadia-api/src/lib/ai/tools/permission-wrapper.ts`. These wrappers:
...
Server code lives in `cascadia-api/src/lib/items/enrichment/` (`enrich-item.ts`, `fetch-source.ts`, `html-to-text.ts`, `limits.ts`); the client side in `cascadia-web/src/components/items/` (`useDropEnrichment.ts`, `enrichment-sources.ts`, `image-payload.ts`, `apply-enrichment.ts`, `DropOverlay.tsx`, `PendingImageStrip.tsx`).
...
| File                                                  | Purpose                                                      |
| ----------------------------------------------------- | ------------------------------------------------------------ |
| `cascadia-api/src/lib/ai/adapters.ts`                 | Provider adapter factory and config loading                  |
| `cascadia-api/src/lib/ai/SessionService.ts`           | Session and message persistence                              |
| `cascadia-api/src/lib/ai/KnowledgeService.ts`         | Schema introspection and system prompt generation            |
| `cascadia-web/src/lib/ai/chat-context.tsx`            | React context for panel state management                     |
| `cascadia-api/src/lib/ai/tools/definitions.ts`        | Read-only tool definitions (Zod schemas)                     |
| `cascadia-api/src/lib/ai/tools/write-definitions.ts`  | Write tool definitions with confirmation schemas             |
| `cascadia-api/src/lib/ai/tools/handlers.ts`           | Read-only tool handler implementations                       |
| `cascadia-api/src/lib/ai/tools/write-handlers.ts`     | Write tool handler implementations                           |
| `cascadia-api/src/lib/ai/tools/permission-wrapper.ts` | Permission checking and audit logging wrapper                |
| `cascadia-api/src/lib/ai/tools/index.ts`              | Tool assembly and exports                                    |
| `cascadia-api/src/lib/db/schema/ai.ts`                | Database schema for sessions, messages, settings, usage logs |
