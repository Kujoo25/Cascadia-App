...
The vault is implemented primarily in `packages/cascadia-api/src/lib/vault/`, with the `FileService` class providing the service layer and the `VaultStorage` interface abstracting the storage backend.
...
The vocabulary lives in `packages/cascadia-commons/src/lib/vault/preview.ts` and is shared by the server
...
Vocabulary lives in `packages/cascadia-commons/src/lib/vault/annotations.ts`, shared by the viewer, the
...
The vocabulary lives in `packages/cascadia-commons/src/lib/vault/file-categories.ts` — labels, badge
...
shared from `packages/cascadia-commons/src/lib/vault/image-files.ts` — as a grid of tiles, and opens any
...
| File                                                                  | Purpose                                                                   |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `packages/cascadia-api/src/lib/vault/services/FileService.ts`         | Core service: upload, download, checkout, checkin, versioning, listing    |
| `packages/cascadia-api/src/lib/vault/storage/types.ts`                | `VaultStorage` interface and configuration types                          |
| `packages/cascadia-api/src/lib/vault/storage/local-storage.ts`        | Local filesystem storage implementation                                   |
| `packages/cascadia-api/src/lib/vault/storage/s3-storage.ts`           | S3-compatible storage implementation                                      |
| `packages/cascadia-api/src/lib/vault/storage/storage-factory.ts`      | Factory for creating storage instances from config                        |
| `packages/cascadia-api/src/lib/vault/utils/file-utils.ts`             | File validation, hashing, category detection, path generation             |
| `packages/cascadia-commons/src/lib/vault/file-categories.ts`          | Shared category vocabulary: labels, badges, filter options                |
| `packages/cascadia-commons/src/lib/vault/preview.ts`                  | Shared preview allowlist: which formats render in-app, and as what        |
| `packages/cascadia-api/src/lib/db/schema/vault.ts`                    | Database schema: `vault_files` and `vault_file_history` tables            |
| `packages/cascadia-web/src/components/vault/FileList.tsx`             | UI component: file listing with lock status and actions                   |
| `packages/cascadia-web/src/components/vault/PdfViewer.tsx`            | UI component: pdf.js viewer (code-split; not exported from the barrel)    |
| `packages/cascadia-web/src/components/vault/FilePreview.tsx`          | UI component: fetches bytes and mounts the viewer for the format          |
| `packages/cascadia-web/src/components/vault/SvgViewer.tsx`            | UI component: vector drawing viewer — zoom, pan, rotate, fullscreen       |
| `packages/cascadia-web/src/components/vault/viewer-controls.ts`       | Zoom stops and fullscreen state shared by the PDF and SVG viewers         |
| `packages/cascadia-web/src/components/vault/FilePreviewDialog.tsx`    | UI component: preview one file in a dialog, opened from `FileList`        |
| `packages/cascadia-web/src/components/vault/ItemFilePreviewPanel.tsx` | UI component: in-page preview of an item's files (Documents' Preview tab) |
| `packages/cascadia-web/src/components/vault/PdfAnnotationLayer.tsx`   | UI component: the markup surface over one rendered PDF page               |
| `packages/cascadia-web/src/components/vault/useFileMarkup.tsx`        | Markup state, mutations, and the comment/label prompt                     |
| `packages/cascadia-web/src/components/vault/FileCategoryMenu.tsx`     | UI component: category picker for correcting a file's category            |
| `packages/cascadia-web/src/components/vault/FileUploadZone.tsx`       | UI component: drag-and-drop file upload                                   |
| `packages/cascadia-api/src/server/routes/files.ts`                    | API route handlers for all file operations                                |
| `packages/cascadia-api/src/server/routes/items.ts`                    | API route handlers for item-scoped file operations                        |
