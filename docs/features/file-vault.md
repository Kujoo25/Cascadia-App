...
The vault is implemented primarily in `cascadia-api/src/lib/vault/`, with the `FileService` class providing the service layer and the `VaultStorage` interface abstracting the storage backend.
...
The vocabulary lives in `cascadia-commons/src/lib/vault/preview.ts` and is shared by the server
...
Vocabulary lives in `cascadia-commons/src/lib/vault/annotations.ts`, shared by the viewer, the
...
The vocabulary lives in `cascadia-commons/src/lib/vault/file-categories.ts` — labels, badge
...
shared from `cascadia-commons/src/lib/vault/image-files.ts` — as a grid of tiles, and opens any
...
| File                                                         | Purpose                                                                   |
| ------------------------------------------------------------ | ------------------------------------------------------------------------- |
| `cascadia-api/src/lib/vault/services/FileService.ts`         | Core service: upload, download, checkout, checkin, versioning, listing    |
| `cascadia-api/src/lib/vault/storage/types.ts`                | `VaultStorage` interface and configuration types                          |
| `cascadia-api/src/lib/vault/storage/local-storage.ts`        | Local filesystem storage implementation                                   |
| `cascadia-api/src/lib/vault/storage/s3-storage.ts`           | S3-compatible storage implementation                                      |
| `cascadia-api/src/lib/vault/storage/storage-factory.ts`      | Factory for creating storage instances from config                        |
| `cascadia-api/src/lib/vault/utils/file-utils.ts`             | File validation, hashing, category detection, path generation             |
| `cascadia-commons/src/lib/vault/file-categories.ts`          | Shared category vocabulary: labels, badges, filter options                |
| `cascadia-commons/src/lib/vault/preview.ts`                  | Shared preview allowlist: which formats render in-app, and as what        |
| `cascadia-api/src/lib/db/schema/vault.ts`                    | Database schema: `vault_files` and `vault_file_history` tables            |
